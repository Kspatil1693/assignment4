...
public class RangeQueries {
	...
	{
		System.out.println("Inside buildSparse table method");
		for(int i=0;i<n;i++)
		{
			table[i][0]=arr[i];
		}
		...
		static long query(int L,int R)
		{
			...
			for(int j=k;j>=0;j--)
			{
				System.out.println("The value of j inside query method:"+j);
				if (L+(1 << j)-1 <=R)
				{
					System.out.println("Inside the if condition of query method");
					answer=answer+table[L][j];
					System.out.println("The value inside table[i][j] is:"+table[L][j]);
					
					System.out.println("The value of answer is:"+answer);
					L+=1<<j;
					System.out.println("The value of L inside the if condition of query method:"+L);
